...
## How the icons replace the crate (FCP-side, SBW-version-independent)

FCP ships its own mechanism so this works even on SBW builds that don't have the
vehicle-icon feature:

* `client/FcpContainerIconDecorator` draws the icon over the container slot in GUIs.
* `mixins/ContainerBlockItemRendererMixin` suppresses SBW's 3D crate in the GUI for
  FCP vehicles that have an icon, so the icon fully replaces it.
* Both are scoped to `fcp:` vehicles only (via the container's `EntityType` tag), so
  SBW's own containers are untouched. The mixin uses `require = 0`, so if it can't
  apply on some SBW version the mod still loads (the icon just draws over the crate).

Icons live at `assets/superbwarfare/textures/vehicle_icon/container/<id>.png` and are
loaded by FCP's own code, so no dependency on SBW's feature or resource lookup.

If a specific vehicle still shows a crate: confirm its PNG exists at that path with
`<id>` exactly the entity path, and that the build included it
(`jar tf build/libs/*.jar | grep vehicle_icon`).

## Auto-regeneration

`processResources` depends on `renderVehicleIcons`, so a normal build regenerates any
missing/changed icons (delete an icon + rebuild to regenerate it). It's guarded by an
`onlyIf` python check: on a machine without python3 + numpy + Pillow the task is skipped
and the committed icons are used as-is (so deleting icons there leaves them missing —
install python or restore them). Run manually any time with `./gradlew renderVehicleIcons`.
