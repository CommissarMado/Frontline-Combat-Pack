...
public class PantsirModel extends FCPVehicleModel<PantsirEntity> {
    ...
    @Override public @Nullable VehicleModel.TransformContext<PantsirEntity> collectTransform(String boneName) {
        VehicleModel.TransformContext<PantsirEntity> steer = WheelRotationTransforms.matchAnyTurn(boneName, 0.6, 30f, "whell2", "whell6", "whell7", "whell8");
        if (steer != null) return steer;
        VehicleModel.TransformContext<PantsirEntity> wheels = WheelRotationTransforms.matchAny(boneName, 0.6, "whell3", "whell9", "whell4", "whell5");
