package frontline.combat.fcp.client.model.Uaz;

import com.atsuishio.superbwarfare.client.model.entity.VehicleModel;
import frontline.combat.fcp.FCP;
import frontline.combat.fcp.client.model.Util.WheelRotationTransforms;
import frontline.combat.fcp.entity.vehicle.Uaz.UAZ3303Entity;
import net.minecraft.resources.ResourceLocation;
import org.jetbrains.annotations.Nullable;
import software.bernie.geckolib.cache.object.GeoBone;
import software.bernie.geckolib.core.animation.AnimationState;

public class UAZ3303Model extends VehicleModel<UAZ3303Entity> {

    @Override
    public void setCustomAnimations(UAZ3303Entity vehicle, long instanceId, AnimationState<UAZ3303Entity> animationState) {
        super.setCustomAnimations(vehicle, instanceId, animationState);
        this.getBone("tent").ifPresent(bone -> setHiddenDeep(bone, !vehicle.hasTent()));
    }

    private static void setHiddenDeep(GeoBone bone, boolean hidden) {
        bone.setHidden(hidden);
        for (GeoBone child : bone.getChildBones()) setHiddenDeep(child, hidden);
    }

    @Override
    public ResourceLocation getModelResource(UAZ3303Entity animatable) {
        return new ResourceLocation(FCP.MODID, "geo/uaz_3303.geo.json");
    }

    @Override
    public boolean hideForTurretControllerWhileZooming() {
        return false;
    }

    @Override
    public @Nullable VehicleModel.TransformContext<UAZ3303Entity> collectTransform(String boneName) {

        // Front wheels (whell/whell2) steer; the geo has no separate "...Turn" sub-bone, so the
        // wheel bone itself is what rolls + pivots.
        VehicleModel.TransformContext<UAZ3303Entity> turn =
                WheelRotationTransforms.matchAnyTurn(boneName, 0.375, 30f,
                        "whell", "whell2");
        if (turn != null) return turn;

        // Rear wheels (whell3/whell4) just roll.
        VehicleModel.TransformContext<UAZ3303Entity> wheels =
                WheelRotationTransforms.matchAny(boneName, 0.375,
                        "whell3", "whell4");
        if (wheels != null) return wheels;

        return super.collectTransform(boneName);
    }
}
