...
// Dual M134 minigun door guns on the Huey skeleton - see HueyM60DoorGunsEntity for the full
// derivation of the dual-custom architecture (both "turret"/"barrel" (LEFT) and "turret2"/
// "barrel2" (RIGHT) get their own custom Matrix4d transform, mirroring VenomGunshipEntity's
// LeftBarrel/RightBarrel pair; neither side relies on SBW's native turret system).
//
// Unlike the M60, each side also has a spinning multi-barrel cluster several bones deeper in the
// chain (LEFT: barrel -> minigun2 -> minigun3 -> minigun13 -> minigun15069; RIGHT: barrel2 ->
// minigun4 -> minigun5 -> minigun6 -> minigun7), matching Venom's GUN2/GUN3 pattern exactly - the
// intermediate bones (minigun2/3/13 and minigun4/5/6) are static cant/realignment rotations baked
// in the geo, and minigun15069/minigun7 are the actual tube clusters that need to visibly spin
// while firing. That spin is purely cosmetic (a local-X rotation of the cluster's own long axis,
// which looks like spinning regardless of what any ancestor rotation does), so it needs no sign
// analysis - only the barrelRotation trackers below and the matching Model.java bone cases.
public class HueyM134DoorGunsEntity extends CamoVehicleBase {
    ...
    // against huey_m134.geo.json - see HueyM60DoorGunsEntity's comment for the method.
    // LEFT_TURRET_POS is the same value this vehicle's TurretPos used to carry, back when the
    // LEFT gun was driven by the native turret system.
    ...
    public HueyM134DoorGunsEntity(EntityType<HueyM134DoorGunsEntity> type, Level world) {
        super(type, world);
