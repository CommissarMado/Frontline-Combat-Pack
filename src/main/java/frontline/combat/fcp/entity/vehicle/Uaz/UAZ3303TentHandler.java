...
/**
 * Toggles the UAZ-3303's "tent" canopy the same way the Ural's TENTY canopy works: right-click
 * the truck with the FCP spray while looking at the canopy's hitbox. If the look ray misses the
 * box the spray is left alone so its default camo repaint still works.
 */
@Mod.EventBusSubscriber(modid = FCP.MODID)
public final class UAZ3303TentHandler {
    ...
    public static void onInteract(PlayerInteractEvent.EntityInteractSpecific event) {
        ...
        if (!(event.getTarget() instanceof UAZ3303Entity truck)) return;

        VehicleEntity vehicle = (VehicleEntity) truck;
        ...
        if (!player.level().isClientSide()) {
            truck.toggleTent();
            player.level().playSound(null, truck.blockPosition(),
